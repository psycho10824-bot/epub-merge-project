...
def _volume_sort_key_from_basename(basename: str):
    ...
    단행본(권) 정렬 전용 키:
      (0, 권번호, 이름)  → 숫자 권
      (1, rank,  이름)  → 특수 권(외전/특별외전/번외/부록/증보/개정)
      (2, 0,    이름)  → 기타
    """
    name = os.path.splitext(basename)[0]

    # 1) '숫자부 숫자권' (예: "1부 2권") 우선 처리
    m = re.search(r"(\d+)\s*부\s*(\d+)\s*권", name)
    if m:
        part = int(m.group(1))   # 부 번호
        vol = int(m.group(2))    # 권 번호
        return (0, part * 1000 + vol, name)

    # 1-2) '숫자 권'만 있는 일반 단행본 (예: "3권")
    m = re.search(r"(\d+)\s*권", name)
    if m:
...
class App:
    def __init__(self, window, dnd_enabled=False):
        ...
        self.window.title("EPUB 합치기(for감자)")
        ...
        self.toc_use_titles_var = tk.BooleanVar(value=False)  # ✅ 가능하면 소제목으로 목차 만들기
        ...
        self._build_ui()
        self._sync_labels()  # 초기 라벨/입력칸 텍스트 동기화
    ...
    def _build_ui(self):
        ...
        self.tabbar.add(self.tab_serial, text="연재본 합치기")
        self.tabbar.add(self.tab_volume, text="단행본 합치기")
        self.tabbar.pack(fill="x")
        self.tabbar.bind("<<NotebookTabChanged>>", self._on_tab_changed)

        # 파일 리스트
        file_frame = ttk.LabelFrame(root, text="병합할 EPUB 목록 (드래그 가능)")
        ...
        ttk.Button(btn_row, text="파일 추가", command=self._add_files).pack(side="left", padx=2)
        ttk.Button(btn_row, text="선택 삭제", command=self._remove_selected).pack(side="left", padx=2)
        ttk.Button(btn_row, text="전체 지우기", command=self._clear_all).pack(side="left", padx=2)
        ttk.Button(btn_row, text="위로 ↑", command=self._move_up).pack(side="left", padx=2)
        ttk.Button(btn_row, text="아래로 ↓", command=self._move_down).pack(side="left", padx=2)
        ttk.Checkbutton(
            btn_row,
            text="파일 이름으로 자동 정렬",
        ...
        # ✅ 새 체크박스: 가능하면 소제목으로 목차 만들기
        ttk.Checkbutton(
            btn_row,
            text="가능하면 소제목으로 목차 만들기",
            variable=self.toc_use_titles_var,
        ).pack(side="left", padx=8)

        # 저장 위치
        out_frame = ttk.LabelFrame(root, text="저장 위치")
        ...
        ttk.Button(out_frame, text="찾아보기", command=self._choose_outdir).pack(side="left", padx=6)

        # 진행 상태
        log_frame = ttk.LabelFrame(root, text="진행 상태")
        ...
        # 병합 시작 버튼
        ttk.Button(root, text="🚀 병합 시작", command=self._start_merge).pack(fill="x", pady=(0, 6))
    ...
    def _start_merge(self):
        ...
        if not self.epub_paths:
            self._log("⚠ 병합할 EPUB 파일이 없습니다.")
            return

        out_dir = self.out_entry.get().strip()
        if not out_dir or not os.path.isdir(out_dir):
            messagebox.showerror("오류", "유효한 저장 위치를 선택해 주세요.")
            return

        mode = self.mode_var.get()
        self._log(f"모드: {mode} / 파일 {len(self.epub_paths)}개")
            ...
            try:
                ...
                if out_path and os.path.exists(out_path):
                    try:
                        ...
                        self._log("⚠ 파일 위치 열기 중 오류가 발생했습니다.")
            except Exception as e:
                self._log(f"❌ 오류 발생: {e}")
                traceback.print_exc()
        else:
            self._log("ℹ 단행본 병합은 아직 구현되지 않았습니다. (정렬/UI만 동작)")
            self.progress["value"] = 100
    ...
    def _merge_serial(self, out_dir: str):
        """
        연재본 EPUB들을 하나로 합치고
        chapter_001, 002 ... 로 리네임 + toc.ncx(차례) 생성.
        - 파일명뿐 아니라 '파일 내부에 기존 목차가 있는지'도 체크하여
          특전/외전 EPUB를 하나의 부모 항목 아래에 묶어준다.
        """
        # --- 0) 파일명에서 시작/끝 화수 및 제목 코어 추출 ---
        def extract_num_for_range(name: str):
            base = os.path.splitext(name)[0]
            patterns = [
                r"\((\d+)\s*~",          # (185~194화)
                r"(\d+)\s*-\s*\d+\s*화", # 1-184화
                r"(\d+)\s*화",           # 595화
                r"(\d+)\s*권",           # 3권
                r"(\d+)",                # 그냥 숫자
            ]
        ...
        # 🔹 목차용 / 파일명용 번호를 분리해서 사용
        toc_start_no = start_no          # toc.ncx에서 증가시키며 쓰는 번호
        range_start_no = start_no        # 최종 파일명에 들어갈 시작 화수
        ...
        self._log(f"임시 작업 폴더: {tmp_root}")
        out_path = None

        try:
            # 1) 첫 번째 EPUB를 베이스로 추출
            base_epub = self.epub_paths[0]
            self._log(f"1/3: 베이스 EPUB 추출: {os.path.basename(base_epub)}")
            base_dir = self._extract_epub_to(tmp_root, base_epub)

            base_opf = self._find_opf_path(base_dir)
            if not base_opf:
                raise RuntimeError("베이스 EPUB에서 content.opf를 찾을 수 없습니다.")
            self._log(f"베이스 OPF: {os.path.relpath(base_opf, base_dir)}")
            ...
            if manifest is None or spine is None:
                raise RuntimeError("베이스 EPUB의 manifest 또는 spine을 찾을 수 없습니다.")

            # --- 1) 베이스 EPUB의 텍스트 디렉터리/아이템 맵 파악 ---
            text_dir = ""
            id_to_item = {}
            ...
            if not text_dir:
                text_dir = ""
            self._log(f"본문 디렉터리 추정: '{text_dir or '(루트)'}'")
            ...
            # --- 2) 베이스 EPUB의 spine 텍스트를 chapter_001... 로 리네임 ---
            ...
            for itemref in list(spine.findall("opf:itemref", ns)):
                ...
                self._log(f"  🔁 기본 본문 리네임: {href} → {new_href}")
            ...
            # 🔹 EPUB(파일)별로 chapter 범위를 기록해서
            #    나중에 특전/외전 파일을 한 묶음으로 보여줄 때 사용
            epub_groups = []
            epub_groups.append({
                "index": 0,                 # 첫 번째 EPUB (베이스)
                "start": 1,
                "end": base_chapter_count,
                "is_special": False,
                "title": title_core,        # 시리즈 기본 제목
            })

            # 베이스에서 사용한 chapter 번호 다음부터 시작
            chapter_index = base_chapter_count + 1

            # --- 3) 나머지 EPUB에서 본문을 가져와서 차례대로 뒤에 붙이기 ---
            for idx, epub_path in enumerate(self.epub_paths[1:], start=2):
                self._log(f"2/3: 추가 EPUB #{idx} 처리 중: {os.path.basename(epub_path)}")
                src_dir = self._extract_epub_to(tmp_root, epub_path)
                src_opf = self._find_opf_path(src_dir)
                if not src_opf:
                    self._log("  ⚠ OPF를 찾지 못해 건너뜁니다.")
                    continue

                # 이 EPUB에서 생성될 chapter_XXX 범위 시작
                ...
                # 이 EPUB에서 실제로 추가된 chapter 범위
                group_end = chapter_index - 1
                if group_end >= group_start:
                    base_epub_name = os.path.basename(epub_path)
                    title_epub = os.path.splitext(base_epub_name)[0]

                    # 🔹 파일명이 특전/외전/스페셜이면 특전 후보
                    is_special_name = bool(
                        re.search(r"(특전|외전|번외|스페셜|special)", base_epub_name, re.IGNORECASE)
                    )
                    # 🔹 파일 안에 자체 toc.ncx(navPoint 2개 이상)가 있으면 특전 후보
                    has_internal_toc = self._epub_has_multi_nav(src_opf)

                    # 🔹 "이름으로 특전" 이거나 "내부 목차 있는 EPUB" 이고
                    #    chapter가 2개 이상인 경우만 묶음 처리
                    is_special_group = (is_special_name or has_internal_toc) and (group_end > group_start)

                    epub_groups.append({
                        "index": idx - 1,          # 0: 첫 EPUB, 1: 두 번째 EPUB ...
                        "start": group_start,
                        "end": group_end,
            ...
            episode_no = toc_start_no  # 🔹 목차용 번호 (파일명용과 분리)

            # chapter_001, chapter_002 ... 순서를 세기 위한 카운터
            chapter_counter = 0

            # 특전/외전 EPUB마다 부모 navPoint를 저장
            group_parents = {}

            def find_group_for_chapter(ch_index: int):
                """해당 chapter 번호가 속한 특전/외전 EPUB 그룹을 찾음."""
            ...
            for itemref in spine.findall("opf:itemref", ns):
                ...
                if is_cover:
                    ...
                    if group:
                        # 🔹 특전/외전 EPUB → 부모 navPoint(예: '... IF 특전')
                        g_idx = group["index"]
                    ...
                    if not label:
                        if episode_no is not None:
                            label = f"{episode_no}화"
                            episode_no += 1
                        else:
                            label = idref
                else:
                    # nav.xhtml, toc.ncx 등은 목차에서 제외
                    continue
            ...
            self._log(f"📌 toc.ncx 생성/갱신 완료: {ncx_path}")

            # --- 5) 수정된 OPF 저장 ---
            tree.write(base_opf, encoding="utf-8", xml_declaration=True)

            # --- 6) 최종 EPUB 파일 이름 구성 및 압축 ---
            # 전부 외전인지 확인 (이 경우 파일명에 (외전) 붙이기)
            all_extra = all('외전' in os.path.basename(p) for p in self.epub_paths)

            if range_start_no is not None and end_no is not None:
                range_part = f"{range_start_no}-{end_no}화"
                extra_part = " (외전)" if all_extra and "외전" not in title_core else ""
                out_filename = f"{title_core} {range_part}{extra_part}.epub"
            else:
                base_name = os.path.basename(self.epub_paths[0])
                name_no_ext, _ = os.path.splitext(base_name)
                out_filename = f"{name_no_ext}_합본.epub"

            out_path = os.path.join(out_dir, out_filename)
            self._log(f"3/3: 최종 EPUB 압축: {out_path}")
            self._zip_epub(base_dir, out_path)

        finally:
            try:
                shutil.rmtree(tmp_root)
                self._log("임시 작업 폴더 삭제 완료")
            except Exception:
                self._log("⚠ 임시 작업 폴더 삭제 중 오류가 발생했지만 계속 진행합니다.")
    ...
    def _epub_has_multi_nav(self, opf_path: str) -> bool:
        """
        주어진 OPF를 가진 EPUB 안에
        toc.ncx(navMap/navPoint)가 2개 이상 있으면 True.
        (기존에 자체 목차가 있는 '특전/외전 미니북' 같은 경우를 잡기 위함)
        """
        try:
            ...
            # navPoint가 2개 이상이면 '자체 목차가 있는 EPUB'로 간주
    ...
    def _extract_chapter_title(self, chapter_path: str) -> str | None:
        """XHTML 파일에서 첫 번째 제목(h1~h6 or chapter-header)을 찾아 텍스트를 리턴."""
        ...
        # 2순위: class에 chapter-header / chapter-title 등이 들어간 요소
        for elem in root.iter():
    ...
    def _append_serial_chapters_from_epub(
        ...
        for itemref in spine.findall("opf:itemref", local_ns):
            ...
            self._log(f"  ➕ chapter {chapter_index:03d} 추가: {new_href}")
    ...
    def _sort_paths(self):
        ...
        if mode == "volume":
            ...
            def num(name: str):
                for pat in [r"\((\d+)\s*~", r"(\d+)\s*화", r"(\d+)\s*권", r"(\d+)"]:
            ...
            def is_extra(name: str) -> int:
                return 1 if re.search(r"(외전|번외|특별편|에필로그)", name) else 0
    ...
    def _choose_outdir(self):
        d = filedialog.askdirectory(title="출력 폴더 선택", initialdir=self.last_output_dir)
        if d:
